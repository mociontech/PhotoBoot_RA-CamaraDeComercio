 ...
 * La IA solo convierte a la persona en pixel art y el poster final se arma en frontend.
 */
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    if (!REPLICATE_API_TOKEN || !REPLICATE_API_TOKEN.startsWith("r8_"))
      return res.status(400).json({ error: "Token no configurado" });
    ...
    const output = await replicate.run(GEMINI_REPLICATE_MODEL, {
      input: {
        prompt,
        image_input: [photo],
        aspect_ratio: "9:16",
    ...
    res.json({ success: true, imageUrl, model: GEMINI_REPLICATE_MODEL });
  } catch (err) {
...
app.listen(PORT, () => {
  ...
  addDebugLog("system", "Servidor iniciado", {
    port: PORT,
    replicate_token_configured: ok,
    datahub: getDatahubStatus(),
  });
  console.log("\n" + "=".repeat(56));
  console.log("  FIFA 2026 - Pixel Art HD - Promologistics");
